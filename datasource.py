...
class Nutrek:
    ...
    def containsAllergen(self, food, allergen):
        ...
        try:
            cursor = self.connection.cursor ()
            cursor.execute("SELECT food_name,ingredients_english FROM Nutrek WHERE (food_name ~ (%s)) AND (ingredients_english ~ (%s))",(food,allergen))
            results = cursor.fetchall()
            return str(results[0])
